...
class CargaMasivaController extends Controller
{
      ...
      public function carga_masiva_productos()
    {
        ...
        $grupos = grupos::where('id','!=',3)->select('id','nombre_grupo')->get();
        // dd($consulta);
        return view('CargaMasiva.cargamasiva_productos',compact('consulta','grupos'));
    }
     ...
     public function subir_carga_masiva_productos(Request $request)
    {
     ...
     public function subir_carga_masiva_productos_grupo_tienda(Request $request)
    {
        //Contador de fallas y exitos de registros
        $GLOBALS['correct'] = 0;
        $GLOBALS['bad'] = 0;
        ...
        Excel::selectSheetsByIndex(0)->load($request->file('excel'), function($reader) use($request) {
          ...
          $reader->each(function($row) use($request) {
              ...
              //Obtener todas las tiendas que pertenezcan al grupo enviado
              $tiendas = tiendas::where('grupo','=',$request->grupo)->get();

              foreach($tiendas as $tienda){ // recorre todas las tiendas

              $search_producto = DB::table('productos')->where([['codigo', '=', $row->codigo],['id_tienda','=', $tienda->id]])->first();

              if(empty($search_producto)){ //Si el producto no existe en esa tienda, la categoria pertenece a la tienda y la subcategoria tambien, se registra el producto

              //Buscar el id de categoria y subcategoria de la tienda en base a los ids en el excel
              $search_category = categorias::where('id','=',$row->id_categoria)->select('categoria')->first();

              $search_category_tienda = categorias::where([['id_tienda','=',$tienda->id],['categoria','=',$search_category->categoria]])->select('id')->first();

              $search_subcategory = subcategorias::where('id','=',$row->id_subcategoria)->select('nombre_categoria')->first();

              $search_subcategory_tienda = subcategorias::where([['id_tienda','=',$tienda->id],['nombre_categoria','=',$search_subcategory->nombre_categoria]])->select('id')->first();

              if(!empty($search_category_tienda) && !empty($search_subcategory_tienda)){ 

             //Nuevo producto 
             $producto = [ 
                'id_tienda'           => $tienda->id,
                'id_categoria'        => $search_category_tienda->id,
                'id_subcategoria'     => $search_subcategory_tienda->id,
                'titulo'              => $row->titulo,
                'codigo'              => $row->codigo, 
                'descripcion'         => $row->descripcion,
                'descuentoOferta'     => 0,
              ...
              if(!empty($producto_save)){
               ...
               $GLOBALS['failed_productos'][$GLOBALS['cont']] = $row->codigo;
                ...
                $GLOBALS['bad']++;
                //Añadir los codigos de productos que fallaron
                $GLOBALS['failed_productos'][$GLOBALS['cont']] = ['codigo' => $row->codigo, 'tienda' => $tienda->slug];
            }
             }else{
                //Contar la cantidad de productos que no se crearon por algun error
                $GLOBALS['bad']++;
      ...
      $message = "<p><strong><i class='fa fa-times'></i> DETALLE DE REGISTROS FALLIDOS:</strong><br><small>>> Estos registros fallidos se pueden dar porque ya existe un producto con esa referencia en la tienda seleccionada o porque el ID de categoria y subcategoria no existan en esa tienda.</small></p>";
      $message .= "<table border='1'><thead><tr><th class='text-center' style='padding: 10px;'>CODIGO PRODUCTO</th><th class='text-center'>NOMBRE TIENDA</th></tr></thead><tbody>";
      foreach($failed_productos as $fail){
          $message .= "<tr><td class='text-center'><b>". $fail['codigo'] ."</b></td><td class='text-center'><b>". $fail['tienda'] ."</b></td></tr>";
      }
      $message .= "</tbody></table>";
